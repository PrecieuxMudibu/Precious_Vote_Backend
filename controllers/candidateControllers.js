...
async function vote_candidate(request, response) {
    ...
    const elector = await Elector.findOne({ token_for_vote: token_for_vote });
    if (elector == null) {
        return response
            .status(404)
        ...
        if (candidate_to_vote == null) {
            ...
            const rounds_for_this_post = await Round.find({
                post_id: candidate_to_vote.post,
            });
            let round_1 = rounds_for_this_post.filter(
                (round) => round.number == 1
            );
            round_1 = round_1[0];

            let round_2 = rounds_for_this_post.filter(
